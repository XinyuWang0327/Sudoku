...
class SudokuGenerator:
    ...
    def print_board(self):
        for indecx, row in enumerate(self.board):
            for col_index, number in enumerate(row):
                end_char='|' if (col_index + 1) % 3 == 0 and col_index != 8 else ' '
                print(number, end=end_char)
            print()
            if (indecx + 1) % 3 == 0 and indecx != 8:
                print('-' * 21)
    ...
    def fill_box(self, row_start, col_start):
        ...
        for i in range(3):
            for j in range(3):
                self.board[int(row_start + i)][int(col_start + j)] = nums.pop(0)
    '''
